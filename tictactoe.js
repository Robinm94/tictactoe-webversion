...
$(function () {
  ...
  function weightedDecision(board) {
    let counter = 0;
  ...
  function winner(board) {
    ...
    for (const line of winlines) {
      ...
      if (
        xwin.length == line.length &&
        xwin.every(function (element, index) {
          return element === line[index];
        })
      ) {
        return X;
      }
      ...
      if (
        owin.length == line.length &&
        owin.every(function (element, index) {
          return element === line[index];
        })
      ) {
        return O;
  ...
  function stateValue(board) {
    ...
    for (let move of listAction) {
      ...
      if (
        (cplayer == X && move_value == 1) ||
        (cplayer == O && move_value == -1)
      ) {
        break;
      }
    }
